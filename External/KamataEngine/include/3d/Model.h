...
public:
	...
	enum class CullingMode {
		kCullingBack = 0, // バックカリング
		kCullingFront,    // フロントカリング
		kCullingNone,     // カリングなし
	...
	struct PipelineSetKey {
		// カリングモード
		CullingMode cullingMode = CullingMode::kCullingBack;
	};

	struct PipelineSetKeyHash {
		size_t operator()(const PipelineSetKey p) const { return static_cast<size_t>(p.cullingMode); }
	};

	class PipelineSetKeyEqual {
	public:
		bool operator()(const PipelineSetKey& left, const PipelineSetKey& right) const { return left.cullingMode == right.cullingMode; }
	};
...
private:
	...
	PipelineSet CreatePipelineSetKey(CullingMode cullingMode) const;

	void CreateAndAddPipelineSet(CullingMode cullingMode);
...
public: // 静的メンバ関数
	...
	static void PreDraw(CullingMode cullingMode = CullingMode::kCullingBack, ID3D12GraphicsCommandList* commandList = nullptr);
